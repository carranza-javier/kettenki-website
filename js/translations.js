const translations = {
  de: {
    ...
    livianapf_walkthrough_scope: 'Jemand versucht, ihr das Passwort zum AWS-Konto zu entlocken. Sie lehnt ab und antwortet grundsätzlich nicht zu Themen, die nicht als sicher hinterlegt sind oder nicht zum Geschäft selbst gehören. Im Hintergrund sorgen Schutzregeln dafür, dass das ganze Gespräch in einem sicheren Rahmen bleibt.',
    ...
    livianapf_did_4: 'Auf beide Zähler wurden bedingte Aktualisierungen angewendet. Ein Lesen mit anschliessendem Schreiben würde zwei gleichzeitige Anfragen zusammen am Limit vorbeilassen; so ist es ein einziger Rundlauf, atomar, und eine abgewiesene Anfrage schreibt überhaupt nichts.',
  ...
  en: {
    ...
    livianapf_context: 'Liviana is one of the products I offer with KettenKI: a chatbot that answers customer questions on a company website. Useful for any small business, project or web portal that wants to give potential new customers the right and precise information faster. Liviana runs on my own brand, KettenKI, and there she does both jobs at once: anyone asking here which solutions I offer is using Liviana at that very moment and watching her work. That sets her apart from the rest of the portfolio: she is not a screenshot and not a repository, she is a product the visitor can try during the visit. Technically that is exactly where the difficulty lies, because Liviana sits on the internet with no sign up: anyone can send requests, and they follow a secured path all the way to Amazon Bedrock.',
    ...
    livianapf_walkthrough_scope: 'Someone tries to get the password to the AWS account out of her. She declines, and as a rule she does not answer on topics that are not set up as safe or do not belong to the business itself. Behind the scenes, guardrails keep the whole conversation inside a safe frame.',
    ...
    livianapf_did_4: 'Conditional updates were applied to both counters. A read followed by a write would let two concurrent requests pass the limit together; this way it is a single round trip, atomic, and a rejected request writes nothing at all.',
